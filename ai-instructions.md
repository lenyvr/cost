# About this project
This is an independent microservice that manages the cost from an application that allows pizza sales.

## Use Cases & Business Rules

- **Register a new measurement unit**: The measurement unit name must be unique and cannot be duplicated.
- **Soft delete a measurement unit**: The measurement unit must exist and be active and cannot be deleted if it has 
...
public class BeanConfiguration {
    ...
    @Bean
    public CreateAccountUseCase createProductUseCase(ProductRepositorySPI productRepositorySPI) {
        return new CreateAccountUseCase(productRepositorySPI);
    }
}
```
